...
bool lcio_pythia8::fill_next_event( Pythia8::Event& pyev, 
  ...
  for (int i = 1; i < pyev.size(); ++i) {
    ...
    mcVec->push_back(lcio_particles[i]); // for progressing check
  }
  ...
  //  double prod_vtx[3] = { p->getVertex()[0] ,
  //			   p->getVertex()[1] ,
  //			   p->getVertex()[2] };
  ...
  //ol doHadr = (pyset == 0) ? m_free_parton_warnings
